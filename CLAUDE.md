...
Phase 1(파서)은 완료됐다. input/output/cache_w/cache_r 네 항목이 픽스처 기대값과
정확히 일치한다. **그 정확도를 깨지 말 것** — 화면을 붙이다가 숫자가 틀리면 Phase 1을
되돌리는 것이다. `py -3.12 -m pytest tests -q`는 계속 통과해야 한다.

기본 은유는 **낚시**다. 저장소 이름 그대로. 패키지 이름 `tokenfishing`은 초기 잔재고 은유가 아니다.

v0.7.0부터 화면 컨셉이 일곱 개다(`themes.py`: 낚시·마을·목장·우주·정원·광산·도시).
**테마는 말과 그림만 바꾼다.** 등급 경계값은 전 테마 공통이고, 집계에는 손대지
않는다 — 테마를 늘려도 숫자가 달라지면 안 된다. `tests/test_themes.py`가 이걸 지킨다.
...
- 세션은 첫 메시지에서 시작, 정확히 5시간 지속. **정시로 내리지 않는다**
  (`FLOOR_TO_HOUR = False`). 한때 True였다 — 다른 구현이 13:36 요청에 대해 13:00을
  내놓길래 맞췄었다. 2026-09-05 공식 사용량 화면과 대조하니 실제 세션 시작은
  **15:17**이었다. 정시가 아니다. **서드파티 구현은 정답지가 아니다** —
...
- burn rate: 최근 1시간의 모든 활성 세션에서 수집 → 분당 토큰.
  **`윈도우 총량 / 윈도우 경과시간`과는 다른 지표다.** 둘 다 맞고 용도가 다르다
  (우리 건 반응성, 저건 소진 예측). 둘을 비교해서 안 맞는다고 버그로 오해하지 말 것
...
픽스처는 전부 손으로 만든 합성 JSONL이고, 함정 네 개를 각각 하나씩 재현한다.
기대값은 테스트 코드에 박혀 있다.

**검증은 반드시 항목별(input / output / cache_w / cache_r)로 따로 한다.**
총합만 보는 테스트는 이 프로젝트에서 무효다 — 함정 1과 2는 오차 방향이 반대라
둘 다 틀리면 총합이 우연히 맞는다.
...
가장 믿을 만한 대조군은 공식 사용량 화면이다. 서드파티 구현은 저쪽도 JSONL만 보고
추측하므로 **둘이 일치한다고 맞는 게 아니다.**
...
- 남의 사용량 모니터 구현에서 분석 로직을 복사하지 말 것. 직접 구현하는 게 이 프로젝트의 목적
- Rich TUI 출력을 파싱하지 말 것. 원본 데이터만 읽는다
- CSV 리포트, 웨어하우스 영속화 같은 기능 확장 금지
- **한도(limit) 퍼센트와 소진 예측을 추정으로 만들어내지 말 것.** 흔한 방식이
  `token_limit`을 P90으로 추정해 `used_percentage` / `pace` / `forecast`를 내놓는 건데,
  실측으로 틀린 게 확인됐다 — 그 추정 **82.6%**, 같은 시점 공식 화면 **35%**.
  두 배 넘게 틀렸고, `pace`와 `forecast`는 그 틀린 값 위에 쌓여 있다

### 어떤 숫자를 넣을지 판단하는 기준

"남들이 주니까 우리도" 는 이유가 아니다. **추측이 들어가는가**로 가른다.
...
- 화면에서 숫자를 만들지 말 것. `state.py`는 `aggregate`가 준 값을 은유로 옮기기만 한다.
  새 집계가 필요하면 `aggregate`에 넣고 테스트로 검증한 뒤 가져다 쓴다

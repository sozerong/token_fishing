...
- P90 한도 추정, CSV 리포트, 웨어하우스 영속화 같은 기능 확장 금지. 숫자는 계속 세 개다: 현재 윈도우 누적 토큰 / burn rate / 리셋까지 남은 시간
- **한도(limit) 퍼센트를 만들어내지 말 것.** 레퍼런스는 `token_limit`을 추정해서 %를 내지만
  그건 P90 추정이고 이 프로젝트가 금지한 것이다. 화면은 절대량과 속도만 보여준다

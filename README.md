...
창 제목에 버전과 데이터 출처가 붙는다 — `token fishing 0.4.0 · 공식`.
`공식`이면 계정 사용량을 그대로 쓰는 중이고, `어림`이면 계산으로 맞춘 값이다.
코드를 고쳤는데 화면이 그대로면 제목의 버전을 먼저 확인해라. 옛 창이 떠 있는 것이다.

숫자가 어디서 왔는지 추적하려면:

```bash
TOKENFISHING_DEBUG=1 tokenfishing
```
